...
class Work(models.Model):
    ...
    FORMULA_CHOICES = [
        ...
        (2, 'Тираж × Цена'),                                # без изменений
        (3, 'Цена × Количество листов × (Количество резов × Коэффициент)'),  # новая логика
...
class WorkPrice(models.Model):
    ...
    # Связь с работой (одна работа – много цен)
    work = models.ForeignKey(
        Work,
        on_delete=models.CASCADE,               # при удалении работы удаляются и цены
        verbose_name='Работа',
        related_name='work_prices',              # позволяет получить work.work_prices.all()
    )

    # Количество листов (опорная точка)
    ...
    # Цена за данное количество листов
    price = models.DecimalField(
        verbose_name='Цена (руб.)',
    ...
    class Meta:
        # Сортировка: сначала по работе, затем по возрастанию количества листов
        ...
        # Гарантируем уникальность: для одной работы не может быть двух одинаковых количеств листов
    ...
    def get_sheets_display(self):
        """Возвращает отформатированное количество листов."""
