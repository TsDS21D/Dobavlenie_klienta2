...
Обновлён для поддержки типа печати.
*/
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Получаем текущий тип печати из мета-тега или из глобальной переменной.
    // В шаблоне можно добавить скрытое поле, но для простоты будем брать из URL.
    function getCurrentPrintType() {
        const urlParams = new URLSearchParams(window.location.search);
        return urlParams.get('print_type') || 'color';
    }
    ...
    if (calculateBtn && arbitraryCopiesInput) {
        calculateBtn.addEventListener('click', function() {
            ...
            formData.append('print_type', getCurrentPrintType());   // Добавляем тип печати
            formData.append('csrfmiddlewaretoken', getCookie('csrftoken'));
    ...
    console.log('Модуль расчета произвольного тиража инициализирован (с поддержкой типа печати)');
});