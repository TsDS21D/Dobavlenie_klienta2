...
# ==================== ПРИНТЕРЫ (с поддержкой типа печати) ====================

@login_required(login_url='/login/')
@never_cache
def index(request):
    """Главная страница для принтеров. Теперь учитывает тип печати (цветная/ч/б)."""
    printers = Printer.objects.all().order_by('name')
    selected_printer_id = request.GET.get('printer_id')
    # НОВОЕ: получаем тип печати из GET-параметра, по умолчанию 'color'
    print_type = request.GET.get('print_type', 'color')
    ...
    if selected_printer_id:
        try:
            ...
            # Фильтруем цены по принтеру и типу печати
    ...
    # Передаём в форму начальное значение типа печати (чтобы в select был выбран нужный)
    form = PrintPriceForm(initial={'printer': selected_printer, 'print_type': print_type})

    if request.method == 'POST':
        form = PrintPriceForm(request.POST)
        if form.is_valid():
            price = form.save()
            messages.success(request, f"Цена добавлена: тираж {price.copies} шт. ({price.get_print_type_display()})")
            # Перенаправляем с сохранением параметров printer_id и print_type
            return redirect(f"/print_price/?printer_id={price.printer.id}&print_type={print_type}")
    ...
    context = {
        ...
        'form': form,
        'device_type': 'printer',
        'current_print_type': print_type,          # передаём в шаблон текущий тип
        'print_type_choices': PrintPrice.PRINT_TYPE_CHOICES,  # список вариантов для переключателя
    }
...
def create_print_price(request):
    """AJAX-создание цены для принтера (с типом печати)."""
    form = PrintPriceForm(request.POST)
    if form.is_valid():
        price = form.save()
...
def delete_print_price(request, price_id):
    """Удаление цены принтера (не зависит от типа)."""
    price = get_object_or_404(PrintPrice, id=price_id)
    printer_id = price.printer.id
    # Для перенаправления нужно знать текущий тип печати – возьмём из POST или GET?
    # Поскольку удаление вызывается через AJAX или POST, в случае обычного POST передадим параметр
    # Но для простоты после удаления перезагрузим страницу с параметрами принтера и текущим типом.
    # В AJAX-запросе перенаправление не нужно.
    print_type = price.print_type
    price.delete()
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return JsonResponse({'success': True})
    messages.success(request, "Цена удалена")
    return redirect(f"/print_price/?printer_id={printer_id}&print_type={print_type}")


@require_POST
@csrf_exempt
def update_print_price(request, price_id):
    """AJAX-обновление цены принтера (inline-редактирование). Тип печати не меняется."""
    price = get_object_or_404(PrintPrice, id=price_id)
    ...
    try:
        if field_name == 'copies':
            ...
            if new_value != price.copies:
                if PrintPrice.objects.filter(
                    printer=price.printer,
                    copies=new_value,
                    print_type=price.print_type
...
def update_printer_interpolation_method(request, printer_id):
    """Обновление метода интерполяции для принтера (общий для всех типов печати)."""
...
def calculate_arbitrary_copies_price(request, printer_id):
    ...
    Теперь учитывает тип печати, переданный в параметре print_type.
    """
    printer = get_object_or_404(Printer, id=printer_id)

    # Получаем тип печати из GET или POST, по умолчанию 'color'
    if request.method == 'POST':
    ...
    # Проверяем, есть ли хотя бы одна цена для данного принтера и типа печати
    if not PrintPrice.objects.filter(printer=printer, print_type=print_type).exists():
        return JsonResponse({'success': False, 'error': f'Нет сохранённых цен для этого принтера (тип: {dict(PrintPrice.PRINT_TYPE_CHOICES).get(print_type, print_type)})'})

    # Вызываем утилиту с указанием типа печати
    cost, markup = get_cost_and_markup_for_printer_and_copies(printer, copies_int, print_type=print_type)
    ...
    type_display = dict(PrintPrice.PRINT_TYPE_CHOICES).get(print_type, print_type)
    ...
    return JsonResponse({
        ...
        'print_type_display': type_display,
    })
...
def get_laminators_list(request):
    ...
    URL: /print_price/laminators/api/get_laminators/
    """
    laminators = Laminator.objects.all().order_by('name')
