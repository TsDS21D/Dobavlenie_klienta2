/*
index.js
JavaScript для главной страницы приложения devices.
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Страница управления принтерами загружена');
...
/**
 * Инициализирует форму добавления принтера.
...
function initForm() {
    ...
    // Автофокус на поле ввода названия принтера при открытии формы
...
function validateForm() {
    const nameInput = document.getElementById('id_name');
    const formatInput = document.getElementById('id_sheet_format');
    const widthInput = document.getElementById('id_width_mm');
    const heightInput = document.getElementById('id_height_mm');
    const marginInput = document.getElementById('id_margin_mm');
    const duplexInput = document.getElementById('id_duplex_coefficient');
    
    // Проверка названия принтера
    const name = nameInput.value.trim();
    if (!name) {
        alert('⚠️ Пожалуйста, введите название принтера!');
        nameInput.focus();
        return false;
    }
    ...
    if (name.length < 2) {
        alert('⚠️ Название принтера должно содержать минимум 2 символа!');
        nameInput.focus();
        return false;
    }
    ...
    if (!format) {
        alert('⚠️ Пожалуйста, введите формат листа!');
        formatInput.focus();
        return false;
    }
    ...
    // Проверка полей (margin)
    const margin = parseInt(marginInput.value);
    if (isNaN(margin) || margin < 0) {
        alert('⚠️ Пожалуйста, введите корректный размер полей (неотрицательное число)!');
        marginInput.focus();
        marginInput.select();
        return false;
    }
    
    // Проверка коэффициента двусторонней печати
    const duplex = parseFloat(duplexInput.value);
    if (isNaN(duplex) || duplex < 1.0) {
        alert('⚠️ Пожалуйста, введите корректный коэффициент (число не меньше 1.0)!');
        duplexInput.focus();
        duplexInput.select();
        return false;
    }
...
/**
 * Инициализирует таблицу принтеров.
...
/**
 * Добавляет поле поиска над таблицей принтеров.
 * Позволяет фильтровать принтеры по названию или формату.
 */
function addTableSearch() {
    const printersSection = document.querySelector('.printers-section');
    const table = document.querySelector('.printers-table');
    
    if (!printersSection || !table) {
        return;
    }
    ...
    searchInput.addEventListener('input', function() {
        ...
        rows.forEach(row => {
            ...
            const formatText = row.querySelector('.col-format .param-badge').textContent.toLowerCase();
            ...
            if (printerName.includes(searchTerm) || 
                formatText.includes(searchTerm) || 
                dimensionsText.includes(searchTerm)) {
                row.style.display = '';
            } else {
...
/**
 * Настраивает подтверждение удаления принтеров.
...
function setupDeleteConfirmation() {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            // Получаем название принтера и формат из строки таблицы
            const row = this.closest('.table-row');
            const printerName = row.querySelector('.printer-name').textContent;
            const printerFormat = row.querySelector('.col-format .param-badge').textContent;
            
            // Показываем подтверждение
            const confirmed = confirm(`Вы уверены, что хотите удалить принтер "${printerName}" (${printerFormat})?\n\nЭто действие нельзя отменить.`);
            
            // Если пользователь отказался, отменяем переход по ссылку
            if (!confirmed) {
                e.preventDefault();
                console.log('Удаление отменено пользователем');
            } else {
                console.log(`Подтверждено удаление принтера: ${printerName} (${printerFormat})`);
                showLoadingIndicator();
            }
        });
    });
 ...
 * Используется при отправке формы или удалении принтера.
 */
