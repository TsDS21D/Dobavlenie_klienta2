 ...
 * - Inline-редактирование любых полей (двойной клик) – включая себестоимость (cost),
 *   наценку (markup_percent), плотность (density), толщину бумаги (paper_thickness),
 *   толщину плёнки (thickness), количество, единицы измерения и т.д.
 ...
 *
 * ИСПРАВЛЕНИЯ (06.04.2026):
 * - Удалено поле price (готовая цена) – теперь цена вычисляется на сервере из cost и markup_percent.
 * - Добавлены поля cost и markup_percent для всех материалов (включая бумагу).
 * - Поле unit теперь общее для всех типов.
 * - Исправлена обработка пустых значений (null) для полей, допускающих NULL.
 * - Добавлена валидация для cost и markup_percent (диапазоны, формат).
 * - В startInlineEdit корректно обрабатываются все поля, включая cost и markup_percent.
 * - В updateElementDisplay добавлено отображение для cost и markup_percent.
 * - Исправлена проблема с отображением null в редакторе.
 * - Улучшена обработка десятичных разделителей (запятая/точка).
 * - Исправлена ошибка ReferenceError: Cannot access 'row' before initialization (row объявлена в начале функции).
 *
 * Автор: разработчик
 * Дата: 2025
 */
...
function loadCategoryTree() {
    ...
    return fetch(`/sklad/category/tree/?type=${sklad_currentMaterialType}`)
        ...
        .then(data => {
            ...
            if (!data || !data.tree) throw new Error('Некорректные данные дерева категорий');
 ...
 * @param {Array} treeData - Массив данных корневых категорий
 */
function renderCategoryTree(treeData) {
    ...
    function createCategoryNode(category, level = 0, isLast = false) {
        ...
        let categoryClass = `tree-item level-${Math.min(level, 3)}`; // level-0, level-1, level-2, level-3 (не более 3)
                ...
                <div class="tree-item-content">
                    ...
                    <div class="tree-name">
                        ...
                        ${category.materials_count > 0 ? 
                            `<span class="material-count" title="${category.materials_count} материалов в этой категории">
...
function highlightSelectedCategory(categoryId) {
    ...
    if (categoryItem) {
        ...
        setTimeout(() => {
            categoryItem.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });
        }, 100);
...
function addSubcategory(parentCategoryId, parentCategoryName) {
    ...
    // Запрашиваем название у пользователя через стандартный диалог prompt
    ...
    formData.append('type', sklad_currentMaterialType);
 ...
 * Для бумаги показываем плотность и толщину бумаги.
 * Для плёнки – толщину плёнки.
 * Общие поля (себестоимость, наценка, единица измерения) видны всегда.
 * 
 * ДОПОЛНЕНО: при изменении типа автоматически перезагружаем список категорий,
 * чтобы показывать только категории, соответствующие выбранному типу.
 */
function updateFormFieldsByType() {
    ...
    if (typeSelect.value === 'paper') {
        // Показываем поля для бумаги, скрываем для плёнки
        if (paperFields) paperFields.style.display = 'block';
        if (filmFields) filmFields.style.display = 'none';
        // Убираем обязательность толщины плёнки
        const thicknessFilm = document.getElementById('material-thickness');
        if (thicknessFilm) thicknessFilm.removeAttribute('required');
    } else {
        // Показываем поля для плёнки, скрываем для бумаги
        if (paperFields) paperFields.style.display = 'none';
        if (filmFields) filmFields.style.display = 'block';
        // Толщина плёнки обязательна
        const thicknessFilm = document.getElementById('material-thickness');
        if (thicknessFilm) thicknessFilm.setAttribute('required', 'required');
    }
    
    // НОВОЕ: обновляем список категорий в соответствии с выбранным типом материала
    updateCategoriesForMaterialForm();
}

/**
 * НОВАЯ ФУНКЦИЯ: загружает категории для выбранного в форме типа материала
 * и обновляет выпадающий список. Также сбрасывает выбранное значение категории.
 */
function updateCategoriesForMaterialForm() {
    const typeSelect = document.getElementById('material-type');
    if (!typeSelect) return;
    
    const selectedType = typeSelect.value; // 'paper' или 'film'
    ...
    // Вызываем AJAX-функцию из модуля skladAJAX, передавая тип материала
    if (window.skladAJAX && typeof window.skladAJAX.loadCategoriesForForm === 'function') {
        // Передаём тип явно, чтобы загрузились только нужные категории
        window.skladAJAX.loadCategoriesForForm(selectedType);
    } else {
        console.error('[SKLAD] skladAJAX.loadCategoriesForForm не найдена');
...
function startInlineEdit(element) {
    ...
    // Для полей, которые могут быть NULL (плёнка и бумага), если значение равно "None" или "null", делаем пустую строку
    if (['cost', 'markup_percent', 'thickness', 'paper_thickness', 'density'].includes(fieldName)) {
    ...
    if (!originalValue || originalValue === 'null' || originalValue === 'undefined') {
        if (fieldName === 'cost' && element.textContent) {
...
function saveInlineEdit(element, newValue) {
    ...
    .then(data => {
        if (data.success) {
            ...
            sklad_currentEditingElement = null;
        } else {
...
function validateInlineInput(fieldName, value) {
    // Поля, которые могут быть пустыми (NULL)
    if (['density', 'paper_thickness', 'thickness'].includes(fieldName) && value === '') {
        return true;
    }
    ...
    if (fieldName === 'name') {
    ...
    } else if (fieldName === 'min_quantity') {
        const minQty = parseFloat(value);
        if (isNaN(minQty) || minQty < 0) {
            showNotification('warning', 'Минимальное количество должно быть неотрицательным числом');
            return false;
        }
        if (!Number.isInteger(minQty)) {
            showNotification('warning', 'Минимальное количество должно быть целым числом');
            return false;
        }
        ...
        if (value.length < 1) {
            showNotification('warning', 'Единица измерения не может быть пустой');
            return false;
        }
    }
    return true;
...
function updateElementDisplay(element, fieldName, value, materialData) {
    // ===== ИСПРАВЛЕНИЕ: переменная row объявлена в начале функции, чтобы быть доступной во всех блоках =====
    const row = element.closest('.table-row');
    ...
    switch (fieldName) {
        case 'name':
            ...
            break;
        case 'cost':
            // Отображаем себестоимость с иконкой рубля
            ...
            // Обновляем отображение цены, так как она изменилась (цена зависит от себестоимости и наценки)
            if (row && materialData && materialData.price_display) {
                const priceSpan = row.querySelector('.price-badge');
                if (priceSpan) {
                    priceSpan.textContent = materialData.price_display;
                }
            }
            break;
        case 'markup_percent':
            // Отображаем наценку с иконкой процента
            if (!value || value === '' || isNaN(parseFloat(value))) {
        ...
        case 'quantity':
            ...
            break;
        case 'min_quantity':
            const minQty = parseInt(value, 10);
            element.innerHTML = minQty;
            element.setAttribute('data-original-value', value);
            // Обновляем статус количества (цвет) у соседнего элемента quantity-badge
            const qtySpan = element.closest('.table-row')?.querySelector('.quantity-badge');
            if (qtySpan && materialData) {
                const currentQty = parseInt(qtySpan.getAttribute('data-original-value') || qtySpan.textContent, 10);
                qtySpan.classList.remove('quantity-zero', 'quantity-low');
                if (currentQty <= 0) qtySpan.classList.add('quantity-zero');
                else if (currentQty <= minQty) qtySpan.classList.add('quantity-low');
            }
            break;
        case 'unit':
            element.innerHTML = escapeHtml(value);
            element.setAttribute('data-original-value', value);
            // Обновляем отображение количества (единица измерения изменилась)
            const quantitySpan = element.closest('.table-row')?.querySelector('.quantity-badge');
            if (quantitySpan && materialData) {
                const qtyNum = parseInt(quantitySpan.getAttribute('data-original-value') || quantitySpan.textContent, 10);
                quantitySpan.innerHTML = `${qtyNum} ${escapeHtml(value)}`;
            }
            // Обновляем цену (единица измерения в цене)
            const priceSpan = element.closest('.table-row')?.querySelector('.price-badge');
            if (priceSpan && materialData && materialData.price_display) {
                priceSpan.textContent = materialData.price_display;
            }
            break;
        case 'thickness':
    ...
    // Сохраняем обновлённый HTML как оригинальный для возможной отмены в будущем
    element.setAttribute('data-original-html', element.innerHTML);
    // Восстанавливаем обработчик двойного клика (на случай, если он был удалён)
    element.setAttribute('ondblclick', 'startInlineEdit(this)');
}
    ...
    if (!localStorage.getItem('sklad_hint_shown')) {
        setTimeout(() => {
            showNotification('info', 
                ...
                'Двойной клик по любому полю для быстрого редактирования.'
            );
