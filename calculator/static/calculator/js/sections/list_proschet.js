...
/**
 * Выбирает просчёт: обновляет выделение в таблице, загружает данные в другие секции,
 * инициализирует обработчик изменения тиража.
 * 
 * @param {HTMLElement} rowElement - DOM-элемент строки таблицы, по которой кликнули
 * @param {string|number} proschetId - ID выбранного просчёта
 */
function selectListProschetRow(rowElement, proschetId) {
    console.log(`🔄 Начинаем выбор просчёта ID: ${proschetId}`);
    
    // ===== 1. ПРЕДОТВРАЩАЕМ ПОВТОРНЫЙ ВЫЗОВ =====
    ...
    // ===== 2. СБРАСЫВАЕМ ОБРАБОТЧИК ТИРАЖА ДЛЯ ПРЕДЫДУЩЕГО ПРОСЧЁТА =====
    resetCirculationHandlerForPreviousProschet();
    
    // ===== 3. СБРАСЫВАЕМ СЕКЦИЮ "ДОПОЛНИТЕЛЬНЫЕ РАБОТЫ" =====
    if (window.additionalWorksSection && typeof window.additionalWorksSection.reset === 'function') {
        window.additionalWorksSection.reset();
        console.log('🔄 Секция "Дополнительные работы" сброшена');
    }
    
    // ===== 4. ОТМЕНЯЕМ ПРЕДЫДУЩИЕ AJAX-ЗАПРОСЫ =====
    ...
    // ===== 5. ОБНОВЛЯЕМ ВЫДЕЛЕНИЕ В ТАБЛИЦЕ =====
    ...
    // ===== 6. ЗАГРУЖАЕМ ДАННЫЕ ПРОСЧЁТА ДЛЯ ДРУГИХ СЕКЦИЙ =====
    ...
    .then(data => {
        ...
        if (signal.aborted || !data || data.proschet.id != listProschetSelectedProschetId) {
            console.warn('⚠️ Данные просчёта устарели, игнорируем');
            return;
        }
        
        if (data.success) {
            const proschetData = data.proschet;
            
            // 6.1. ОБНОВЛЯЕМ СЕКЦИЮ "КЛИЕНТ" (напрямую, без лишнего запроса)
            // Используем существующую функцию updateClientInterface, которая работает с объектом proschetData
            updateClientInterface(proschetData);
            
            // 6.2. ОБНОВЛЯЕМ СЕКЦИЮ "ИЗДЕЛИЕ"
            updateProductSectionProschetTitle(rowElement);
            if (window.productSection && typeof window.productSection.updateFromProschet === 'function') {
                window.productSection.updateFromProschet(proschetData);
            }
            
            // 6.3. ОБНОВЛЯЕМ СЕКЦИЮ "ПЕЧАТНЫЕ КОМПОНЕНТЫ"
            updatePrintComponentsSectionForProschet(proschetId, rowElement, signal);
            
            // 6.4. ОБНОВЛЯЕМ СЕКЦИЮ "ЦЕНА"
            ...
            // ===== 7. ИНИЦИАЛИЗИРУЕМ ОБРАБОТЧИК ТИРАЖА =====
            if (window.circulationChangeHandler && typeof window.circulationChangeHandler.updateProschetId === 'function') {
                const circulation = proschetData.circulation || 1;
                window.circulationChangeHandler.updateProschetId(proschetId, circulation);
                console.log(`✅ Обработчик тиража обновлён: просчёт ${proschetId}, тираж ${circulation}`);
            } else {
                console.warn('⚠️ circulationChangeHandler не найден или не имеет метода updateProschetId');
            }
            
            console.log(`✅ Все секции инициализированы для просчёта ID: ${proschetId}`);
        } else {
            console.error('❌ Ошибка загрузки данных просчёта:', data.message);
            showListProschetNotification('Ошибка загрузки данных просчёта', 'error');
        }
    })
    .catch(error => {
        if (error.name === 'AbortError') return;
        console.error('❌ Ошибка сети при загрузке просчёта:', error);
        showListProschetNotification('Ошибка сети при загрузке просчёта', 'error');
    })
    .finally(() => {
...
/**
 * Обновляет данные в секции "Изделие" (тираж, название) и инициализирует обработчик изменения тиража.
...
function updateProductSectionData(proschetId, signal) {
    ...
    if (signal && signal.aborted) {
        console.log(`ℹ️ Запрос для просчёта ${proschetId} был отменён, пропускаем`);
        return;
    }
    
    // Проверяем, что это всё ещё тот же просчёт
    if (proschetId != listProschetSelectedProschetId) {
        console.warn(`⚠️ Пропускаем: запрос для просчёта ${proschetId}, но выбран ${listProschetSelectedProschetId}`);
        return;
    }
    ...
    .then(data => {
        ...
        if (proschetId != listProschetSelectedProschetId) {
            console.warn(`⚠️ Данные для старого просчёта ${proschetId}, игнорируем`);
            return;
        }
        
        if (data.success && data.proschet) {
            const proschet = data.proschet;
            
            // Обновляем секцию "Изделие"
            if (window.productSection && typeof window.productSection.updateFromProschet === 'function') {
                window.productSection.updateFromProschet(proschet);
            }
            
            // ===== ГЛАВНОЕ ИСПРАВЛЕНИЕ: ОБНОВЛЯЕМ ОБРАБОТЧИК ТИРАЖА =====
            // Теперь, когда мы точно знаем тираж из данных просчёта,
            // инициализируем обработчик изменения тиража.
            if (window.circulationChangeHandler && typeof window.circulationChangeHandler.updateProschetId === 'function') {
                const circulation = proschet.circulation || 1;
                window.circulationChangeHandler.updateProschetId(proschetId, circulation);
                console.log(`✅ Обработчик тиража обновлён: просчёт ${proschetId}, тираж ${circulation}`);
            } else {
                console.warn('⚠️ circulationChangeHandler не найден или не имеет метода updateProschetId');
    ...
    .catch(error => {
        if (error.name === 'AbortError') return;
        console.error('❌ Ошибка сети при загрузке данных просчёта:', error);
    });
}
