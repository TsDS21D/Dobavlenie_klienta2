...
let productIsSaving = false;                   // Флаг для предотвращения повторных сохранений
...
function productSaveCirculationEdit() {
    ...
    if (productIsSaving) {
        console.log('ℹ️ Уже выполняется сохранение, пропускаем');
        return;
    }
    ...
    if (!productValidateCirculation(newValue)) {
        ...
        inputElement.focus();
        inputElement.select();
        return; // здесь не блокируем saving, т.к. не отправляли запрос
    }
    
    const circulationNumber = parseInt(newValue, 10);
    productIsSaving = true;  // устанавливаем блокировку перед отправкой
...
function productCancelCirculationEdit() {
    ...
    productDeactivateCirculationEdit();
    productEditingProschetId = null;
    productIsSaving = false;   // сброс флага
