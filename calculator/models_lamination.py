...
ДОБАВЛЕНА СТОРОНА ЛАМИНАЦИИ (односторонняя/двусторонняя).
...
class Laminate(models.Model):
    ...
    # ===== НОВОЕ ПОЛЕ: сторона ламинации =====
    side = models.CharField(
        verbose_name='Сторона ламинации',
        max_length=10,
        choices=SIDE_CHOICES,
        default='single',
        help_text='Односторонняя или двусторонняя ламинация. При двусторонней расход плёнки удваивается.'
    )
    ...
    total_price = models.DecimalField(
        ...
        default=Decimal('0.00'),
        validators=[MinValueValidator(Decimal('0.00'))],
        help_text='(цена ламинации + цена плёнки × коэффициент стороны) × количество листов'
    )
    ...
    def __str__(self):
        if self.is_enabled and self.laminator and self.film:
            ...
            return f"Ламинация для {self.print_component.number}: {self.laminator.name} + {self.film.name} ({side_display})"
    ...
    def recalculate_price(self, sheet_count):
        ...
        Вызывается при изменении количества листов, ламинатора, плёнки,
        стороны ламинации или включении/выключении.

        ИСПРАВЛЕНИЕ: используется film.get_price() вместо film.price.
        """
        ...
        # Если нет ламинатора или плёнки – расчёт невозможен, обнуляем всё
        if not self.laminator or not self.film:
            self.laminator_cost = Decimal('0.00')
            self.laminator_markup = Decimal('0.00')
            self.laminator_price = Decimal('0.00')
            self.film_price = Decimal('0.00')
        ...
        if self.film and self.film.type == 'film':
            # ИСПРАВЛЕНИЕ: используем метод get_price() вместо удалённого поля price
            # get_price() возвращает розничную цену с учётом наценки (для плёнки)
            self.film_price = self.film.get_price().quantize(Decimal('0.01'))
        else:
            self.film_price = Decimal('0.00')

        # ===== НОВОЕ: УЧЁТ СТОРОНЫ ЛАМИНАЦИИ =====
        # При двусторонней ламинации расход плёнки удваивается
        if self.side == 'duplex':
            film_price_for_calc = self.film_price * 2
        else:
            film_price_for_calc = self.film_price

        # 3. Общая стоимость = (цена ламинации + цена плёнки с учётом стороны) × количество листов
        total = (self.laminator_price + film_price_for_calc) * Decimal(str(sheet_count))
